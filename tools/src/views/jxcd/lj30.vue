<template>
  <div class="root">
    ...
    <mu-paper class="demo-paper" :z-depth="4" id="mypaper">
      <div class="title">
        ...
        <div class="condition">
          ...
          <div class="myinput">
            <mu-text-field v-model="ia" label="轴惯性矩Ia="  label-float>m^4</mu-text-field>
          </div>
          <div class="myinput">
    ...
    <mu-paper class="demo-paper" :z-depth="4" id="mypaper">
      ...
      <img src="../assets/lj30.png" alt width="50%" />
      <div class="center">
        <p
          class="para"
        >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;1、长度系数，与螺杆的端部结构有关 2、螺杆临界载荷 Fcr用于校核螺杆的稳定性，防止失稳。稳定的条件为Fcr/F ≥2.5~4。不能满足此要求时，可增大螺杆中径 d1。Fcr 的求解分为三种情况，本公式适用于 μ1*l/i>85~90。</p>
      </div>
    </mu-paper>
  </div>
</template>
...
export default {
  ...
  methods: {
    cal() {
        ...
        let la = parseFloat(this.la);
        let u1 = parseFloat(this.u1);
        let l = parseFloat(this.l);
        
      let result = (Math.PI*Math.PI*e*la)/Math.pow(u1*l,2)
